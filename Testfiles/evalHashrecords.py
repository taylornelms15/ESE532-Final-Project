import sys
import os
import re

import numpy as np
import matplotlib.pyplot as plt

import pdb

fileNameDefault = "Franklin_hashrecord.txt"

rindBox = [ ]


def hashKey(key):
    """
    Hashes the 20(?)-bit key into a 10(?)-bit value
    Thought: most variance around lowest 8 bits (the "char" value), and next 8 bits (the "curTableRow" for the lower end), esp the lower of those (9 and 10)
    """
    vlo4    = (key)         & 0xF
    lo4     = (key >> 4)    & 0xF
    hi4     = (key >> 8)    & 0xF
    vhi4    = (key >> 12)   & 0xF
    vvhi5   = (key >> 16)   & 0x1F
    sub8    = ((vvhi5 << 3) ^ vlo4)
    sub8    = rindBox[sub8]
    hiT4    = vhi4 ^ vlo4
    midT4   = (vlo4 ^ hi4 ^ vvhi5) & 0xF
    loT4    = lo4 ^ hi4
    retval  = 0
    retval  ^= loT4
    retval  ^= (midT4 << 3)
    retval  ^= (sub8 << 1)
    retval  ^= (hiT4 << 6)
    retval  &= 0x3FF
    return retval

def graphBuckets(buckets):
    """
    makes bar graph for 1024 "buckets" of numResults
    """
    index = np.arange(len(buckets))
    plt.figure(figsize=(30, 5))
    plt.margins(0.0, tight = True)
    plt.bar(index, buckets)
    plt.xticks(np.arange(0, 1025, 16))
    plt.show()

def evalHashDistribution(records):
    keys    = [x[0] for x in records]
    hashes  = [hashKey(x) for x in keys]
    buckets = [0] * 1024
    total = 0
    for hashval in hashes:
        buckets[hashval] += 1
        total += 1
    maxVal = max(buckets)
    minVal = min(buckets)
    idealMax = total / 1024
    print("Total num records: %s" % total)
    print("\tMaximum bucket val: %s" % maxVal)
    print("\tMinimum bucket val: %s" % minVal)
    print("\tIdeal avg bucket val: %s" % idealMax)
    print("Hash distribution ratio: %s" % (maxVal / idealMax))
    graphBuckets(buckets)


def main():
    records = []
    if len(sys.argv) > 1:
        fileName = sys.argv[1]
        print("Filename: %s" % fileName)
    else:
        fileName = fileNameDefault
    with open(fileName, "r") as f:
        for line in f.readlines():
            line = line.strip()
            key, val = re.split(" ", line)
            key = int(key, 16)
            val = int(val, 16)
            records.append((key, val))

    evalHashDistribution(records)




if __name__ == "__main__":
    main()
