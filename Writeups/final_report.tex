...
\newline\par
So where, in our core logic inside the PL, could the bottleneck be? It is possible that it is the compute within the \texttt{SHA} function; there are certainly a lot of loops to perform for each
...
\newline\par
How can we be sure these implementations would meet the \texttt{10Gbps} requirement? Frankly, we cannot. However, what we can do is make some estimates.
\par
