...
\textbf{Axis:} $$, Memory requirement for sending data from CDC to SHA and LZW
\newline
...
\newline
\textbf{Equation for Benefit:} Saves memory from using MAX_CHUNK($8$KB) amount of memory to just a few bytes

\item%3

\textbf{Axis:} $$, Granularity of data that can be sent from one unit to the next in the application flow
\newline
