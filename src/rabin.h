...
int rabin_next_chunk_HW(uint8_t in_window[WINSIZE], uint8_t out_window[WINSIZE], unsigned int count, unsigned int pos, uint64_t digest, uint8_t buf[MAXINPUTFILESIZE], uint8_t chunk[MAXSIZE], uint64_t out_table[256], uint64_t mod_table[256], unsigned int len);

struct chunk_t *rabin_finalize(struct rabin_t *h);

int rabin_next_chunk_SW(struct rabin_t *h, uint8_t buf[MAXINPUTFILESIZE], uint8_t chunk[MAXSIZE], uint64_t out_table[256], uint64_t mod_table[256], unsigned int len);
#endif
